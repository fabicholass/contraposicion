...
(() => {
    ...
    function loadPackage(metadata) {
        ...
        if (typeof window === 'object') {
            ...
            if (typeof process === 'undefined' && typeof location !== 'undefined') {
        ...
        var REMOTE_PACKAGE_NAME = Module.locateFile ? Module.locateFile('neverball.data', '') : 'neverball.data';
        ...
        function runWithFS(Module) {
            ...
            Module.preloadResults['js/neverball.data'] = { fromCache: false };
            if (fetched) {
...
function findWasmBinary() {
    ;
    if (!isDataURI('neverball.wasm')) {
        return locateFile('neverball.wasm');
    }
    return 'neverball.wasm';
}
var wasmBinaryFile;
...
var ASM_CONSTS = {
    ...
    387212: () => window.innerWidth,
    387242: () => window.innerHeight
};
    ...
    node_ops: {
        getattr(node) {
            var attr = {
                ...
                blksize: 4096,
                blocks: Math.ceil(attr.size / attr.blksize)
            };
            ;
            ;
            ;
...
var IDBFS = {
    ...
    getLocalSet: (mount, callback) => {
        var entries = { path: { timestamp: stat.mtime } };
...
var FS = {
    ...
    analyzePath(path, dontResolveLastLink) {
        ...
        try {
            ...
            null = lookup.path;
            null = lookup.node;
            null = PATH.basename(path);
            lookup = FS.lookupPath(path, { follow: !dontResolveLastLink });
            false = true;
            null = lookup.path;
            null = lookup.node;
            null = lookup.node.name;
            false = lookup.path === '/';
        } catch (e) {
            0 = e.errno;
        }
...
var SYSCALLS = {
    ...
    doStat(func, path, buf) {
        ...
        tempI64 = [
            ...
            (tempDouble = Math.floor(atime / 1000), +Math.abs(tempDouble) >= 1 ? tempDouble > 0 ? +Math.floor(tempDouble / 4294967296) >>> 0 : ~~+Math.ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)
        ];
        ...
        tempI64 = [
            ...
            (tempDouble = Math.floor(mtime / 1000), +Math.abs(tempDouble) >= 1 ? tempDouble > 0 ? +Math.floor(tempDouble / 4294967296) >>> 0 : ~~+Math.ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)
        ];
        ...
        tempI64 = [
            ...
            (tempDouble = Math.floor(ctime / 1000), +Math.abs(tempDouble) >= 1 ? tempDouble > 0 ? +Math.floor(tempDouble / 4294967296) >>> 0 : ~~+Math.ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)
        ];
...
function ___syscall_getdents64(fd, dirp, count) {
    try {
        ...
        var idx = Math.floor(off / 280);
        while (idx < stream.getdents.length && pos + 280 <= count) {
            var id;
            var type;
            ...
            tempI64 = [
                (idx + 1) * 280 >>> 0,
                (tempDouble = (idx + 1) * 280, +Math.abs(tempDouble) >= 1 ? tempDouble > 0 ? +Math.floor(tempDouble / 4294967296) >>> 0 : ~~+Math.ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)
            ];
...
var __mktime_js = function (tmPtr) {
    var ret = (() => {
        ...
        if (dst < 0) {
            ...
            if (dst > 0 != (dstOffset == guessedOffset)) {
                ...
                if (typeof Browser.setImmediate == 'undefined') {
                    if (typeof setImmediate == 'undefined') {
                        ...
                        var Browser_setImmediate_messageHandler = event => {
                            if (event.data === 'setimmediate' || event.data.target === 'setimmediate') {
...
var Browser = {
    ...
    init() {
        ...
        var imagePlugin = {
            ...
            handle: function imagePlugin_handle(byteArray, name, onload, onerror) {
        ...
        var audioPlugin = {
            canHandle: function audioPlugin_canHandle(name) {
            ...
            handle: function audioPlugin_handle(byteArray, name, onload, onerror) {
                var done = false;
                    ...
                    function encode64(data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            while (leftbits >= 6) {
                                ...
                                leftbits -= 6;
                                ret += 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'[curr];
                            }
                        }
                        if (leftbits == 2) {
                            ret += 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'[(leftchar & 3) << 4];
                            ret += '==';
                        } else {
                            if (leftbits == 4) {
                                ret += 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'[(leftchar & 15) << 2];
                                ret += '=';
                            }
                        }
                        return ret;
    ...
    fakeRequestAnimationFrame(func) {
        ...
        if (Browser.nextRAF === 0) {
            Browser.nextRAF = now + 16.666666666666668;
        } else {
            while (now + 2 >= Browser.nextRAF) {
                Browser.nextRAF += 16.666666666666668;
            }
        }
    ...
    if (ret === undefined) {
        ...
        case 'object':
            if (result === null) {
                ...
                if (result instanceof Float32Array || result instanceof Uint32Array || result instanceof Int32Array || result instanceof Array) {
